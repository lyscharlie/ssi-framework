...
public class Money implements Serializable, Comparable {
	 ...
	 * 对象比较。
	 * <p>
	 * 比较本对象与另一对象的大小。 如果待比较的对象的类型不是<code>Money</code>，则抛出<code>java.lang.ClassCastException</code>。 如果待比较的两个货币对象的币种不同，则抛出<code>java.lang.IllegalArgumentException</code>。
	 ...
	 * 货币比较。
	 * <p>
	 * 比较本货币对象与另一货币对象的大小。 如果待比较的两个货币对象的币种不同，则抛出<code>java.lang.IllegalArgumentException</code>。 如果本货币对象的金额少于待比较货币对象，则返回-1。 如果本货币对象的金额等于待比较货币对象，则返回0。 如果本货币对象的金额大于待比较货币对象，则返回1。
	 *
	 * @param other 另一对象。
	 * @return -1表示小于，0表示等于，1表示大于。
	 ...
	 * 货币比较。
	 * <p>
	 * 判断本货币对象是否大于另一货币对象。 如果待比较的两个货币对象的币种不同，则抛出<code>java.lang.IllegalArgumentException</code>。 如果本货币对象的金额大于待比较货币对象，则返回true，否则返回false。
	 *
	 * @param other 另一对象。
	 ...
	 * 货币乘法。
	 * <p>
	 * 返回一个新的货币对象，币种与本货币对象相同，金额为本货币对象的金额乘以乘数。 本货币对象的值不变。如果相乘后的金额不能转换为整数分，使用缺省的取整模式 <code>DEFUALT_ROUNDING_MODE</code>进行取整。
	 *
	 * @param val 乘数
	 * @return 相乘后的结果。
	 */
	 ...
	 * 货币除法。
	 * <p>
	 * 返回一个新的货币对象，币种与本货币对象相同，金额为本货币对象的金额除以除数。 本货币对象的值不变。如果相除后的金额不能转换为整数分，使用缺省的取整模式 <code>DEFAULT_ROUNDING_MODE</code>进行取整。
	 *
	 * @param val 除数
	 * @return 相除后的结果。
	 */
	...
	/**
	 * 根据地区得到显示用的货币单位 如Locale是zh_CN，返回“元”，如Locale是zh_HK返回“圆”
	 *
	 * @param e 需要得到的地区
	 * @return 显示用的单位，比如“元”，“美元”
	 */
